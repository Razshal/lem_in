...
/*   Updated: 2018/05/02 17:37:34 by mfonteni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int							add_lem(int lem_num, char *room_name,
							t_recurse *infos);
int							add_room(char *room_name,
							int roomtype, t_recurse *infos);
int							add_link(char *line, t_recurse *infos);
int							is_end_room(char *room_name,
							t_recurse *infos);
int							lists_init(t_recurse **infos, t_map **map);
void						delete_struct(t_recurse *infos);
t_path						*get_path(t_room_list *rl);
int							moove_lems(t_lem_list *lem,
							t_room_list *rl, t_map *map);
t_attr_paths				*calc_lems_by_path(t_lem_list *lem,
							int lem_nbr, int path_nbr);
